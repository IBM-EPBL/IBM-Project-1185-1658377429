...
conn=ibm_db.connect("DATABASE=bludb;HOSTNAME=19af6446-6171-4641-8aba-9dcff8e1b6ff.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;PORT=30699;SECURITY=SSL;SSLServerCertificateDigiCertGlobalRootCA.crt;PROTOCOL=TCPIP;UID=ncl66473;PWD=r2Ciii0CiSVri1qq;", "", "")
...
def register():
  if request.method == 'POST':
    ...
    if not username or not email or not phoneno or not gender or not password:
      return render_template('reg.html',error='Please fill all fields')
    hash=bcrypt.hashpw(password.encode('utf-8'),bcrypt.gensalt())
    query = "SELECT * FROM USER_REGISTRATION WHERE email=? OR password=?"
    ...
    if not isUser:
      insert_sql = "INSERT INTO USER_REGISTRATION(username, email, phoneno, password,gender) VALUES (?,?,?,?,?)"
      ...
      return render_template('reg.html',success="You can login")
    else:
      return render_template('reg.html',error='Invalid Credentials')

  return render_template('reg.html',name='Home')
